...
export default function View(){
        ...
        <>
            ...
            <select className="select w-full max-w-xs shadow mb-5 mt-5" onChange={changeActiveMonth}>
                {months.map((month, index) => {
                    return <option key={index} value={month} selected={month === activeMonth}>{dayjs(month).format("MMMM YYYY")}</option>
                })}
            </select>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-5 mt-5`}>
                {activities[activeMonth].days.map((day, index) => {
                    ...
                    return <div key={day.date} className={`relative p-10 rounded-2xl shadow text-center ${bgDay}`}>
                        ...
                        {workDay ?
                            <div>
                                <div className="absolute top-3 left-3">
                                    <svg onClick={()=>document.getElementById(`day_modal_${index}`).showModal()} xmlns="http://www.w3.org/2000/svg" className={`w-5 h-5 ${infoColor}`} viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM216 336h24V272H216c-13.3 0-24-10.7-24-24s10.7-24 24-24h48c13.3 0 24 10.7 24 24v88h8c13.3 0 24 10.7 24 24s-10.7 24-24 24H216c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-208a32 32 0 1 1 0 64 32 32 0 1 1 0-64z"/></svg>
                                </div>
                                <dialog id={`day_modal_${index}`} className="modal bg-base-primary">
                                    <div className="modal-box">
                                        ...
                                        <div className="text-left">
                                            ...
                                            {calculatePercentages(day.refactorActivities).map((a) => {
                        ...
                        <p className="italic">{day.countries.length > 0 ? <p>{day.countries.join(", ")}</p> : ""}</p>
                    </div>
                })}
            </div>
        </>
    ...
    // const activeMonthData = useMemo(() => data[activeMonth], [data, activeMonth]);
    //
    // const changeActiveMonth = (e) => {
    //     setActiveMonth(e.target.value);
    // }
    ...
    //
    //
    //         <p className="text-xl font-bold">Всего за месяц: {activeMonthData.totalDistance} km / Средняя за день: {activeMonthData.averageDistance.toFixed(2)} km</p>
    //
    //
    //     </>
    // );
